...
const TheHeader = () => {
  ...
  const filteredItems = getFilteredItems(query, movies1?.results);
  const movies2 = fetchSecondMovies();
  const [isSearched, setIsSearched] = useState(false);
  return (
    <header className="flex z-50 text-black absolute top-[20px] justify-around items-center w-[70%]">
      <Link href="/">
      ...
      <input
        ...
        placeholder="Serch"
        className="h-[35px] outline-none text-gray-400 w-[300px] bg-gray-100 p-[10px] rounded-2xl"
      />

      {isSearched && (
        <div
          onMouseLeave={() => setIsSearched(false)}
          className="z-50 mt-[300px] gap-[10px] px-[40px] absolute  flex-col top-[-200px] left-[100px] border-2 w-[500px] text-white  max-h-[300px] rounded-2xl flex p-[30px] bg-[#121212] overflow-y-scroll min-h-[300px]"
        >
